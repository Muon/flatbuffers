...
fn verify_vector_of_unions<'a, 'b, 'c>(
    ...
    // Either both vectors must be present and have equal length, or both must be absent.
    ...
    for (enum_pos, union_pos) in enum_vector_range.zip(value_vector_range.step_by(SIZE_UOFFSET)) {
        let enum_value: usize = verifier.get_u8(enum_pos)?.into();
